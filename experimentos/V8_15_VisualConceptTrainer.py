...
def plot_heatmap(matrix, y_labels, expert_sizes, title, filename):
    ...
    plt.ylabel("Conceito Visual")
    plt.tight_layout()
    plt.savefig(f'graficos/{filename}')
    plt.close()

def main():
    print("Iniciando V8_15: Treinamento de Especialização por Conceito Visual (Dataset Real)...")
    
    data_dir = r"F:\themes_dataset_ouro"
    meta_path = os.path.join(data_dir, "metadata_ouro.json")
    
    if not os.path.exists(meta_path):
        print("Erro: Metadata não encontrada.")
        return
    ...
    tokenizer = VisualConceptTokenizer()
    ...
    dataset = VisualConceptDataset(metadata, data_dir, tokenizer, transform)
    ...
    with torch.no_grad():
        for batch_idx, (images, seqs, items) in enumerate(dataloader):
            if batch_idx > 25: break # Avalia uns ~400 componentes
    ...
    for category in counts.keys():
        ...
        plot_heatmap(matrix, labels, expert_sizes, f"Especialização por {category.upper()}", f"expert_{category}_heatmap.png")
        print(f"[Artefato Crítico] Heatmap gerado em: graficos/expert_{category}_heatmap.png")
