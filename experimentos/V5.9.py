...
# =========================
# V5.9 — Contextual Gate + Residual Routing + Confidence Sharpening
# =========================
class V59_MoE:
    def __init__(self, input_dim, hidden_sizes, output_dim,
        ...
        self.gW2 = rng.randn(32, 16) * 0.1
        self.gb2 = np.zeros(16)
        self.gW3 = rng.randn(16, self.n_experts) * 0.1
        self.gb3 = np.zeros(self.n_experts)

        # Adam optimizer state for gate
    ...
    def forward(self, X, temperature=1.0):
        ...
        for b in range(B):
            idx = topk_idx[b]
            # Residual Routing: idx[-1] is Top1, idx[-2] is Top2, idx[-3] is Top3
            gate[b, idx[-1]] = 1.0
            gate[b, idx[-2]] = 0.5
            gate[b, idx[-3]] = 0.5
            gate[b] /= np.sum(gate[b]) # normalize to 1.0 sum
    ...
    def train_step(self, X, y, temperature=1.0):
        ...
        # Cross entropy grad + load balance
        d_logits_scaled = (gate_probs - target_probs) / B + self.balance_weight * load_balance_grad
        d_logits = d_logits_scaled / temperature # apply temperature chain rule
        
        # Backprop through Contextual Gate
        d_gW3 = g2.T @ d_logits
...
def evaluate(model, X, y, output_dim, temperature=0.7):
    logits, gate, _, _, _, _, gate_probs, _ = model.forward(X, temperature)
...
def run():
    ...
    for name, fn in datasets.items():
        ...
        model = V59_MoE(input_dim=X.shape[1],
        ...
        acc, class_routing, gate, gate_probs = evaluate(model, X, y, out_dim, temperature=0.7)
    ...
    with open("resultados_finais/v5_9_scalable_routing.json", "w") as f:
