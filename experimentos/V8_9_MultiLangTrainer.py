...
class MultiLangTokenizer:
    ...
    def _split_code(self, text):
        # Quebra simples mantendo as tags de linguagem intactas
        tokens = re.findall(r'<LANG_[A-Z]+>|</?[a-zA-Z0-9]+>|style=\{{[^}]+\}}|className="[^"]+"|href="[^"]+"|[a-zA-Z0-9]+|[^\s\w]', text)
...
class MultiLangDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image, torch.tensor(encoded_html, dtype=torch.long), lang

def collate_fn(batch, pad_idx):
    images, seqs, langs = zip(*batch)
    ...
    return images, padded_seqs, langs

def main():
    print("Iniciando V8_9: Análise de Especialização por Linguagem de Programação...")
    
    data_dir = "dataset_multilang"
    ...
    for m in meta_files:
        with open(m, 'r') as f:
            all_codes.append(json.load(f)["target_code"])
    ...
    dataset = MultiLangDataset(data_dir, tokenizer, transform)
    ...
    print("\nTreinamento concluído. Extraindo Mapa de Especialização por Linguagem...")
    ...
    with torch.no_grad():
        for batch_idx, (images, seqs, langs) in enumerate(dataloader):
            if batch_idx > 10: 
                break
            ...
            for b in range(inputs.size(0)):
                lang = langs[b]
                if lang not in lang_expert_counts:
                    lang_expert_counts[lang] = {i: 0 for i in range(5)}
                ...
                for t in range(inputs.size(1)):
                    ...
                    lang_expert_counts[lang][winning_expert] += 1

    print("\n" + "="*60)
    print("RELATÓRIO ESTATÍSTICO: ESPECIALIZAÇÃO POR LINGUAGEM")
    print("="*60)
    
    matrix_to_plot = []
    lang_labels = []
    
    for lang, expert_counts in lang_expert_counts.items():
        total_tokens = sum(expert_counts.values())
        print(f"\n[{lang.upper()}] (Baseado em {total_tokens} tokens avaliados):")
        ...
        for expert_id, count in expert_counts.items():
            ...
            if pct > 0:
                print(f"  -> Expert {expert_id} ({expert_sizes[expert_id]} neurônios) processou {pct:.1f}% do código.")
    ...
    sns.heatmap(matrix_to_plot, annot=True, fmt=".2f", cmap="Blues", 
                xticklabels=[f"E{i} ({s}n)" for i, s in enumerate(expert_sizes)], 
                yticklabels=lang_labels)
    plt.title("Especialização por Linguagem: Paradigma vs Expert")
    plt.xlabel("Experts (Capacidade)")
    plt.ylabel("Linguagem / Paradigma")
    plt.tight_layout()
    plt.savefig('graficos/expert_lang_heatmap.png')
    plt.close()
