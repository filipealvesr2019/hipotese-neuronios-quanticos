...
# =========================
# V5.5 — Reward + FLOP + Specialized Experts MoE
# =========================
class V55_MoE:
    def __init__(self, input_dim, hidden, output_dim,
                 n_experts=5, top_k=2, lr=0.01, ema=0.9, diversity_weight=0.01, flop_weight=1e-6):
    ...
    # =========================
    # Train step with reward + FLOPs + diversity
    # =========================
    def train_step(self, X, y):
        ...
        diversity_loss = -self.diversity_weight * np.sum(p_mean * np.log(p_mean + 1e-9))

        # FLOPs penalty
        flop_est = X.shape[0] * X.shape[1] * self.hidden * self.n_experts
        flop_loss = self.flop_weight * flop_est

        total_loss = self.loss(logits, y) + diversity_loss + flop_loss
...
def run():
    ...
    for name, fn in datasets.items():
        ...
        model = V55_MoE(input_dim=X.shape[1],
                        hidden=32,
        ...
        flop = X.shape[0] * X.shape[1] * model.hidden * model.n_experts
    ...
    with open("resultados_finais/v5_5_specialized_experts.json", "w") as f:
