...
# =========================
# V5.3C — Routing + MI + Diversity MoE
# =========================
class V53CMoE:
    def __init__(self, input_dim, hidden, output_dim,
                 n_experts=5, top_k=2, seed=42, lr=0.01, ema=0.9, diversity_lambda=0.1):
    ...
    # =========================
    # Train step with credit, MI & diversity
    # =========================
...
def run():
    ...
    for name, fn in datasets.items():
        print(f"\n===== DATASET: {name} =====")
        X, y = fn()
        model = V53CMoE(input_dim=X.shape[1], hidden=32, output_dim=len(np.unique(y)))
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(0, len(X), batch_size):
                ...
                loss_val, gate, expert_perf = model.train_step(X[batch], y[batch])
    ...
    with open("resultados_finais/v5_3C_routing_mi_diversity.json", "w") as f:
